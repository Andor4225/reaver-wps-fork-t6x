...
#define PBT "%.2X"
	if(delim)
		snprintf(buf, MAC2STR_BUF_SIZE, PAT PAT PAT PAT PAT PBT, PRT(0), PRT(1), PRT(2), PRT(3), PRT(4), mac[5]);
	else
		snprintf(buf, MAC2STR_BUF_SIZE, PBT PBT PBT PBT PBT PBT, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
